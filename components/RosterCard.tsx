      ...
      <div className="flex items-start gap-3 sm:gap-4">
        <div className="flex-shrink-0">
          {user.avatarUrl ? (
            ...
            <img
              src={user.avatarUrl}
              alt={user.name}
              className="w-12 h-12 sm:w-14 sm:h-14 rounded-full object-cover border border-stone-300"
            />
          ) : (
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-stone-900 text-stone-100 flex items-center justify-center font-medium text-base sm:text-lg border border-stone-300">
              {initials}
            </div>
          )}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="font-medium text-stone-900 text-base sm:text-lg break-words">{user.name}</h3>
