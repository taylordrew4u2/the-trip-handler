...
export async function sendApprovalEmail(email: string, name: string) {
  await resend.emails.send({
    from: FROM_ADDRESS,
    to: email,
    subject: "You're approved for the trip",
    html: `
      <div style="font-family: ui-sans-serif, system-ui, sans-serif; max-width: 560px; margin: 0 auto; color: #1c1917;">
...
export async function sendRejectionEmail(email: string, name: string) {
  try {
    await resend.emails.send({
      from: FROM_ADDRESS,
      to: email,
      subject: "Update on your trip application",
      html: `
        <div style="font-family: ui-sans-serif, system-ui, sans-serif; max-width: 560px; margin: 0 auto; color: #1c1917;">
          <p style="font-size: 12px; text-transform: uppercase; letter-spacing: 2px; color: #888; margin: 0 0 8px;">The Trip Handler</p>
          <h1 style="font-size: 22px; margin: 0 0 12px;">Hey ${name},</h1>
          <p style="font-size: 14px; line-height: 1.5; color: #444;">Thanks for applying to the trip. Unfortunately we&apos;re not able to fit you in this round. Reach out to admin directly if you have questions.</p>
        </div>
      `,
    });
  } catch (err) {
...
export async function sendCancellationEmail(email: string, name: string) {
  try {
    await resend.emails.send({
      from: FROM_ADDRESS,
      to: email,
      subject: "Your trip spot has been cancelled",
      html: `
        <div style="font-family: ui-sans-serif, system-ui, sans-serif; max-width: 560px; margin: 0 auto; color: #1c1917;">
          <p style="font-size: 12px; text-transform: uppercase; letter-spacing: 2px; color: #888; margin: 0 0 8px;">The Trip Handler</p>
          <h1 style="font-size: 22px; margin: 0 0 12px;">Hey ${name},</h1>
